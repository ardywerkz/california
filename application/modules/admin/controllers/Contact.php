...
class Contact extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['contact'] = $this->setting_model->getMessage($config["per_page"], $data['page'], $data['searchFor'], $data['orderField'], $data['orderDirection']);
        $config['total_rows'] = $this->setting_model->count_message($config["per_page"], $data['page'], $data['searchFor'], $data['orderField'], $data['orderDirection']);
    ...
    public function vew_message($id)
    {
        if ($id) {
            $data['view'] = $this->db->get_where('contact_us', array('id' => $id))->row_array();
            // echo '<pre>';
            // print_r($data['vew']);
            $data['subview'] = $this->load->view('contact/view', $data, TRUE);
            $this->load->view('layouts', $data);
        }
    }
}
